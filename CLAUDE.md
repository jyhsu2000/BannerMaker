...
- **Minecraft 支援範圍**：1.21.0 ~ 26.2（最低版本鎖在 `plugin.yml` 的 `api-version: 1.21`）。
- **不回頭支援更舊的 MC 主版本**（1.20、1.19 …）。舊版使用者應下載歷史 tag（v2.4.0 for 1.20、v2.3.2 for 1.17–1.19 等）。
- 支援範圍內的版本差異由 XSeries 與少數 workaround 處理（如 `MessageService` 對 Paper 1.21.7+ ClickEvent 的繞道）。自 26.2 起 MC 改用年份版號，跨版號相容性同樣仰賴 XSeries 保持在新版。
- 註：v3.0.3（XSeries 13.7.1）的實測版本為 1.21.11 與 26.2；最低支援的 1.21.0 未做回歸驗證。
